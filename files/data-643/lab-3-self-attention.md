...
2. Scores \(S = QK^{\top}/\sqrt{2}\). Print \(S\).

3. \(A = \mathrm{softmax}(S, \dim=-1)\). Rows must sum to 1. Heatmap \(A\) with token names `The, cat, sat, on`.

4. Output \(Y = A V\). Which token’s output is closest (cosine) to `cat`? Guess before you compute.
...
2. In one sentence: why would a GPT-style model be cheating without this mask?
...
You do not need to train it. Copy `attn.in_proj_weight` is optional. The point of (1)–(2) is the algebra. Write three sentences: query vs key vs value, what a row of \(A\) is, and why Week 2’s hidden state is no longer the only memory.
